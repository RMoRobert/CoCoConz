...
@Field static Map<Long,Integer> lastCT = [:]
@Field static Map lightEffects = [0: "None", 1:"Color Loop"]

metadata {
   definition (name: "CoCoConz Group", namespace: "RMoRobert", author: "Robert Morris", importUrl: "https://raw.githubusercontent.com/RMoRobert/CoCoConz/main/drivers/cococonz-group-driver.groovy") {
      capability "Actuator"
   ...
   preferences {
      ...
      input name: "enableVerifyCT", type: "bool", title: "Enable CT verification (recommended for ZHA \"color dimmable light\" devices", defaultValue: true
 ...
 * Hubitat DNI is created in format "Cz/AppId/Grooup/DeCONZDeviceID", so just
...
void createEventsFromMap(Map bridgeCommandMap = state.nextCmd, boolean isFromBridge = false) {
   ...
   String colorMode = bridgeMap["colormode"] ?: device.currentValue("colorMode")?.toLowerCase()
   log.warn "1 colorMode = $colorMode"
   if (isFromBridge && bridgeMap["colormode"] == "xy") {
      colorMode == "ct"
      logDebug("In XY mode but parsing as CT")
   }
   log.warn "2 colorMode = $colorMode"
   ...
   bridgeMap.each {
      switch (it.key) {
         ...
         case "bri":
            ...
            break
         case "colormode":
            log.warn "in colormode.... colorMode=$it.value"
            eventName = "colorMode"
            eventValue = (it.value == "hs" ? "RGB" : "CT")
            log.warn "eventValue = $eventValue"
            eventUnit = null
            if (device.currentValue(eventName) != eventValue) {
         ...
         case "hue":
            ...
            if (isFromBridge && colorMode != "hs") {
                  if (device.currentValue("hue") != null) {
                     logDebug("Skipping hue, colorMode, and color name event creation because light not in hs mode")
                     break
                  }
                  else {
                     logDebug("Not in hs mode, but not skipping hue event because currently null")
                  }
            }
            if (device.currentValue(eventName) != eventValue) {
               if (!isOn && isFromBridge && colorStaging && (state.nextCmd?.get("hue") || state.nextCmd?.get("sat") || state.nextCmd?.get("ct"))) {
         ...
         case "sat":
            ...
            if (isFromBridge && colorMode != "hs") {
                  if (device.currentValue("saturation") != null) {
                     logDebug("Skipping saturation, colorMode and color name event creation because light not in hs mode")
                     break
                  }
                  else {
                     logDebug("Not in hs mode, but not skipping saturation event because currently null")
                  }
            }
            if (device.currentValue(eventName) != eventValue) {
               if (!isOn && isFromBridge && colorStaging && (state.nextCmd?.get("hue") || state.nextCmd?.get("sat") || state.nextCmd?.get("ct"))) {
...
void sendBridgeCommand(Map customMap = null, boolean createHubEvents=true) {    
   ...
   Map params = [
      uri: data.fullHost,
      path: "/api/${data.username}/groups/${getHueDeviceNumber()}/state",
...
void parseSendCommandResponse(resp, data) {
   ...
   if (checkIfValidResponse(resp) && data) {
      ...
      if (data["ct"] && settings["enableVerifyCT"] != false) {
         lastCT[device.id] = data["ct"]
         runIn(3, "verifyCT") //see verifyCT() doc comment for why
      }
   }
   else {
...
/**
  * Workaround for deCONZ ZHA/ZB3 (or bulb?) oddity where "colormode: ct" but no "ct" specified. Will set to
  * last CT sent from Hubitat if this happens. Not sure if needed for groups...
  */
void verifyCT() {
   if (enableDebug) log.debug "verifyCT()... (lastCT =  ${lastCT[device.id]})"
   Integer 
   if (lastCT[device.id] && device.currentValue("colorMode") == "CT") {
      Integer targetCT = Math.round(1000000/lastCT[device.id])
      // Give a ±2 Kelvin threshold for rounding errors, etc.
      if (device.currentValue("ct") < targetCT - 2 || device.currentValue("ct") > targetCT + 2) {
         createEventsFromMap([ct: lastCT[device.id]])
      }
   }
}
